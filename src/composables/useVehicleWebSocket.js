...
export function useVehicleWebSocket() {
  ...
  function startHeartbeat() {
    ...
    heartbeatTimer = window.setInterval(() => {
      ...
      missedPongs += 1
      if (missedPongs >= HEARTBEAT_MISS_LIMIT) { socket.close(); return }
      socket.send('ping')
  ...
  function connect() {
    ...
    try {
      ...
      socket.onopen = () => { status.value = 'open'; reconnectAttempts = 0; missedPongs = 0; socket.send('ping'); startHeartbeat() }
      socket.onmessage = event => {
        try {
          if (String(event.data).trim().toLowerCase() === 'pong') { missedPongs = 0; return }
      ...
      socket.onclose = () => { stopHeartbeat(); socket = null; scheduleReconnect() }
    } catch (error) {

...
3. 将任务已保存的 GCJ-02 起终点直接传给高德生成驾车 polyline；同一任务缓存该路线，
...
创建运输任务必须由前端高德 POI 组件连同地点名称提交以下字段；数据库列与查询响应
保持可空，以兼容历史任务：
...
任务与 planned-route 接口坐标统一为 GCJ-02，不做二次转换。用户界面只选择真实地点，
不提供手工输入经纬度的表单。历史数据库任务坐标为空时仍可查询，但新建任务缺少任一
坐标会返回 400。

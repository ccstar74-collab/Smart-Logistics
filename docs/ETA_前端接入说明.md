...
`ETA_UPDATED` **不是固定频率的心跳消息**：后端只在 ETA 计算结果成功写库后才推送，
写库受节流参数控制（ETA 变化不足阈值时不写库也不推送）。

因此：

- 不保证每个扫描周期都收到一条 `ETA_UPDATED`，任务越多、ETA 越稳定，消息间隔越长。
- 长时间收不到消息只表示该任务 ETA 没有变化，前端不要据此判断连接断开或任务异常。
- 页面展示的数据新鲜度以消息中的 `etaCalculatedAt` 为准。

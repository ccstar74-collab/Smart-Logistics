...
| `tool` | 展示云端业务工具回答；结构化数据位于 `toolData` |

## 云端业务数据工具

车辆、货物、运输任务和告警均通过对话接口查询：

```http
POST /api/chat
```

查询例如“TEST-A001 现在在哪？”时，智能体只读访问云端 Spring Boot/MySQL 业务 API。响应示例：

```json
{
  "mode": "tool",
  "answer": "云端业务系统查询结果：……最新定位 暂无。",
  "toolData": {
...
车辆坐标来自云端 Vehicle API 的 `lastLongitude`、`lastLatitude` 和 `lastUpdatedAt`。
这些字段为 `null` 时表示云端暂无定位；前端和智能体都不应自行补造坐标。
...
- 尚未接入用户登录和权限；
- 已接入云端车辆、货物、运输任务和告警的只读列表接口；
- 尚未接入云端 InfluxDB 的实时 GPS 与历史轨迹接口，云端 Vehicle API 没有坐标时不要声称已获得实时位置；

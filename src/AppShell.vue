...
const { unreadCount, refreshUnreadCount, markRead, receiveNotification, compensateUnread } = useNotifications()
...
function normalizeRealtimeNotification(item) {
  ...
  return {
    ...
    title: item?.title || (type.includes('DISPATCH') ? '新调度指令' : type.includes('ALARM') ? '新告警通知' : '新消息'),
    content: item?.content ?? item?.message ?? '',
    targetPath: item?.targetPath ?? item?.target_path ?? (type.includes('DISPATCH') ? '/dispatch' : type.includes('ALARM') ? '/alarms' : '/notifications')
  }
}
const notificationWs = useNotificationWebSocket(async rawItem => {
  const item = normalizeRealtimeNotification(rawItem)
  if (!receiveNotification(item)) return
  ElNotification({
    title: item.title || '新消息',
    message: item.content || '',
    type: ({ INFO: 'info', SUCCESS: 'success', WARNING: 'warning', ERROR: 'error' })[item.level] || 'info',
    duration: 6000,
    onClick: async () => {
      try { await markRead(item.id) } catch { /* 列表页会再次同步权威状态 */ }

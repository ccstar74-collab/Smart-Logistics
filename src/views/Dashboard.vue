...
const statusText = { WAITING:'待运输', TRANSPORTING:'运输中', COMPLETED:'已完成', ABNORMAL:'异常', CANCELLED:'已取消', SENT:'待确认', PENDING:'待确认', ACKNOWLEDGED:'已确认', EXECUTING:'执行中', REJECTED:'已拒绝' }
const { state } = useAuth()
...
const driverCommands = ref([])
const driverRoute = ref([])
const driverInitialRoute = ref([])
...
const warehouseMapVehicles = computed(() => warehouseVehicles.value.flatMap(vehicle => {
  ...
  const routeStart = warehouseRoutesMap[String(task?.id)]?.active?.[0] ?? warehouseRoutesMap[String(task?.id)]?.initial?.[0]
...
function taskRouteItems(tasks, routesMap) {
  return tasks.map(task => ({
    id: task.id,
    path: routesMap[String(task.id)]?.active ?? [],
    initialPath: routesMap[String(task.id)]?.initial ?? [],
    startPoint: taskEndpoint(task, 'start'),
    endPoint: taskEndpoint(task, 'end'),
...
async function loadAllTaskRoutes(tasks, routesMap) {
  const entries = await Promise.all(tasks.map(async task => {
    try {
      ...
      const versions = extractList(versionsResult).slice().sort((a, b) => Number(a?.routeVersion ?? a?.route_version ?? Infinity) - Number(b?.routeVersion ?? b?.route_version ?? Infinity))
      return [String(task.id), { active: routeList(activeResult), initial: routeList(versions.find(route => routeList(route).length >= 2) || []) }]
    } catch { return [String(task.id), { active: [], initial: [] }] }
  }))
  Object.keys(routesMap).forEach(key => { delete routesMap[key] })
  entries.forEach(([id, routes]) => { routesMap[id] = routes })
}
...
function taskEndpoint(task, prefix) {
...
const ownerOriginalStartPoint = computed(() => taskEndpoint(selectedOwnerTask.value, 'start'))
const ownerOriginalEndPoint = computed(() => taskEndpoint(selectedOwnerTask.value, 'end'))
const ownerRoute = ref([])
const ownerInitialRoute = ref([])
...
async function loadDriverData(){
  ...
  try{
    const [currentResult,vehicleResult,cargoResult,commandResult]=await Promise.all([api.transportTasks.current().catch(()=>null),api.vehicles.list({page:1,pageSize:100}),api.cargos.list({page:1,pageSize:100}),api.dispatchCommands.mine({page:1,pageSize:20})])
    ...
    driverVehicles.value=extractList(vehicleResult);driverCargos.value=extractList(cargoResult);driverCommands.value=extractList(commandResult)
...
async function loadOwnerRoutes(task) {
  if (!task?.id) { ownerRoute.value = []; ownerInitialRoute.value = []; return }
  const [activeResult, versionsResult] = await Promise.all([
    api.transportTasks.plannedRoute(task.id),
    api.transportTasks.routes.list(task.id).catch(() => [])
  ])
  ownerRoute.value = routeList(activeResult)
  const versions = extractList(versionsResult).slice().sort((a, b) => Number(a?.routeVersion ?? a?.route_version ?? Infinity) - Number(b?.routeVersion ?? b?.route_version ?? Infinity))
  ownerInitialRoute.value = routeList(versions.find(route => routeList(route).length >= 2) || [])
}
async function loadDriverRoutes(task) {
  if (!task?.id) { driverRoute.value = []; driverInitialRoute.value = []; return }
  const [activeResult, versionsResult] = await Promise.all([
    api.transportTasks.plannedRoute(task.id),
    api.transportTasks.routes.list(task.id).catch(() => [])
  ])
  driverRoute.value = routeList(activeResult)
  const versions = extractList(versionsResult).slice().sort((a, b) => Number(a?.routeVersion ?? a?.route_version ?? Infinity) - Number(b?.routeVersion ?? b?.route_version ?? Infinity))
  driverInitialRoute.value = routeList(versions.find(route => routeList(route).length >= 2) || [])
}
...
watch(() => selectedOwnerTask.value?.id, () => { loadOwnerRoutes(selectedOwnerTask.value).catch(()=>{ownerRoute.value=[];ownerInitialRoute.value=[]}); refreshSelectedTaskDetail(selectedOwnerTask.value, ownerTasks).catch(()=>{}); if (role.value === 'OWNER') refreshPositionSnapshot().catch(()=>{}) }, { immediate: true })
...
watch(() => driverTask.value?.id, () => { loadDriverRoutes(driverTask.value).catch(()=>{driverRoute.value=[];driverInitialRoute.value=[]}); refreshSelectedTaskDetail(driverTask.value, driverApiTask).catch(()=>{}); if (role.value === 'DRIVER') refreshPositionSnapshot().catch(()=>{}) }, { immediate: true })
onMounted(async()=>{
...
<template>
  ...
  <template v-if="role==='OWNER'">
    <SkeletonScreen v-if="ownerLoading" variant="dashboard" />
    <template v-else>
    ...
    <article class="panel home-map-panel owner-map-panel">
      <div class="panel-title"><div><h2>我的货物运输路线</h2></div><RouterLink class="link-btn" to="/tracking">查看追踪</RouterLink></div>
      <div class="owner-map-wrap">
        <AMapView :selectedVehicleId="selectedOwnerVehicle ? String(selectedOwnerVehicle.id) : ''" :selectedTaskId="selectedOwnerTaskId" :externalVehicles="ownerMapVehicles" :initial-route="ownerInitialRoute" initial-route-coordinate-system="GCJ02" :plannedRoute="ownerRoute" planned-route-coordinate-system="GCJ02" :route-start-label="selectedOwnerTask?.startLocation || ''" :route-end-label="selectedOwnerTask?.endLocation || ''" :route-start-point="ownerOriginalStartPoint" :route-end-point="ownerOriginalEndPoint" route-endpoint-coordinate-system="GCJ02" :showFacilities="false" :showTrack="false" focus-selected />
        <div v-if="ownerPopupVisible && selectedOwnerTask" class="owner-map-task-card owner-hover-task-card" :style="{ left: `${ownerPopupPosition.x + 18}px`, top: `${Math.max(12, ownerPopupPosition.y - 18)}px` }">
          <div class="order-row"><span>运单号</span><strong class="order-number">{{selectedOwnerTask.taskNo}}</strong></div><div><span>货物</span><strong>{{ownerCargoMap[selectedOwnerTask.cargoId]?.name||`#${selectedOwnerTask.cargoId}`}}</strong></div><div><span>状态</span><strong class="blue">{{statusText[selectedOwnerTask.status] || selectedOwnerTask.status}}</strong></div><div><span>车辆</span><strong>{{selectedOwnerVehicle?.plateNumber||`#${selectedOwnerTask.vehicleId}`}}</strong></div><div><span>任务进度</span><strong class="blue">{{ownerTaskProgress}}%</strong></div><el-progress :percentage="ownerTaskProgress" :show-text="false" :stroke-width="8"/><small>{{ownerMapVehicles.length?'最新定位来自实时位置接口':'车辆暂无有效定位'}}</small>
        </div>
      </div>
    ...
    <section class="owner-dashboard-grid">
      <article class="panel role-table-card">
        <div class="panel-title"><div><h2>我的运输任务</h2></div><RouterLink class="link-btn" to="/eta">查看全部</RouterLink></div>
        <div class="table-wrap"><table><thead><tr><th>运单号</th><th>目的地</th><th>ETA</th><th>状态</th></tr></thead><tbody>
          <tr v-for="t in myOwnerTasks" :key="t.id" :class="{selected:t.id===selectedOwnerTask?.id}" @click="selectOwnerTask(t.id)"><td>{{t.taskNo}}</td><td>{{t.endLocation}}</td><td>{{dateText(t.estimatedArrivalTime)}}</td><td><span class="task-status">{{statusText[t.status] || t.status}}</span></td></tr><tr v-if="!myOwnerTasks.length"><td colspan="4">暂无本人运输任务</td></tr>
        </tbody></table></div>
      </article>
      <article class="panel role-table-card">
        <div class="panel-title"><div><h2>近期告警</h2></div><RouterLink class="link-btn" to="/alarms">查看全部</RouterLink></div>
  ...
  <template v-else-if="role==='DRIVER'">
    ...
    <section class="driver-top" v-loading="driverLoading">
      <article class="panel task-card">
        <div class="panel-title"><div><h2>当前任务</h2></div><span v-if="driverTask" class="badge">{{statusText[driverTask.status] || driverTask.status}}</span></div>
        <dl v-if="driverTask">
          <div><dt>运单号</dt><dd>{{driverTask.taskNo}}</dd></div><div><dt>起点</dt><dd>{{driverTask.startLocation}}</dd></div>
          <div><dt>司机</dt><dd>{{state.currentUser?.name || state.currentUser?.realName || '—'}}</dd></div><div><dt>终点</dt><dd>{{driverTask.endLocation}}</dd></div><div><dt>货物</dt><dd>{{driverCargoRecord?.name || `#${driverTask.cargoId}`}}</dd></div>
      ...
      <article class="panel home-map-panel">
        <div class="panel-title"><div><h2>路线与车辆位置</h2></div></div>
        <AMapView :selectedVehicleId="driverVehicle?.vehicle_id || ''" :selected-task-id="driverTask?.id" :externalVehicles="driverVehicle ? [driverVehicle] : []" :initial-route="driverInitialRoute" initial-route-coordinate-system="GCJ02" :plannedRoute="driverRoute" planned-route-coordinate-system="GCJ02" :routeStartLabel="driverTask?.startLocation || ''" :routeEndLabel="driverTask?.endLocation || ''" :route-start-point="taskEndpoint(driverTask, 'start')" :route-end-point="taskEndpoint(driverTask, 'end')" route-endpoint-coordinate-system="GCJ02" :showFacilities="false" focus-selected @select="selectVehicle" />
      </article>
    </section>
    <section class="driver-bottom">
      <article class="panel">
        <div class="panel-title"><div><h2>状态上报</h2></div></div>
      ...
      <article class="panel">
        <div class="panel-title"><div><h2>调度指令</h2></div><RouterLink class="link-btn" to="/dispatch">查看全部</RouterLink></div>
        <div class="command-cards driver-dashboard-commands">
          <RouterLink v-for="command in driverCommands.slice(0,4)" :key="command.id" to="/dispatch" class="driver-command-card">
            <div><strong>{{command.content || command.message || '调度指令'}}</strong><span>{{dateText(command.createdAt || command.sentAt)}}</span></div>
            <small>{{command.taskNo || `任务 #${command.taskId || '—'}`}}</small><b>{{statusText[command.status || 'SENT']}}</b>
          </RouterLink>
          <div v-if="!driverCommands.length" class="empty-state">暂无调度指令</div>
        </div>
      </article>
    </section>
    </div>
  ...
  <template v-else-if="role==='WAREHOUSE_MANAGER'">
    ...
    <article class="panel home-map-panel warehouse-map-panel">
      <div class="panel-title"><div><h2>本仓库货物运输地图</h2></div><select v-model="selectedWarehouseTaskId" class="mini"><option v-for="task in warehouseTasks" :key="task.id" :value="task.id">{{task.taskNo}}</option></select></div>
    ...
    <section class="warehouse-main">
      <article class="panel role-table-card">
        <div class="panel-title"><div><h2>车辆列表</h2></div><RouterLink class="link-btn" to="/vehicles">车辆管理</RouterLink></div>
        <div class="table-wrap" v-loading="warehouseLoading"><table><thead><tr><th>车牌号</th><th>车型</th><th>状态</th><th>所属司机</th></tr></thead><tbody><tr v-for="v in warehouseVehicles.slice(0,6)" :key="v.id"><td>{{v.plateNumber}}</td><td>{{v.type}}</td><td><span class="task-status">{{v.status==='TRANSPORTING'?'运输中':v.status==='IDLE'?'空闲':v.status}}</span></td><td>{{v.driverName || v.driver?.name || (v.driverId ? '已绑定司机' : '未绑定司机')}}</td></tr><tr v-if="!warehouseVehicles.length"><td colspan="4">暂无云端车辆</td></tr></tbody></table></div>
      </article>
      <article class="panel warehouse-outbound-entry">
        <div class="panel-title"><div><h2>货物出库</h2></div></div>
  ...
  <template v-else-if="role==='DISPATCHER'">
    ...
    <article class="panel home-map-panel">
      <div class="panel-title"><div><h2>调度运输路线地图</h2></div><div class="role-live-status"><select v-model="selectedDispatcherTaskId" class="mini"><option v-for="task in dispatcherTasks" :key="task.id" :value="task.id">{{task.taskNo}}</option></select><RouterLink class="link-btn" to="/tracking">车辆监控</RouterLink></div></div>
    ...
    <section class="dispatcher-bottom">
      <article class="panel role-table-card">
        <div class="panel-title"><div><h2>待处理告警</h2></div><RouterLink class="link-btn" to="/alarms">查看全部</RouterLink></div>
      ...
      <article class="panel"><div class="panel-title"><div><h2>快捷操作</h2></div></div><div class="quick-actions">
  ...
  <template v-else>
    ...
    <article class="panel home-map-panel">
      <div class="panel-title"><div><h2>全局运输监控</h2></div><select v-model="selectedAdminTaskId" class="mini"><option v-for="task in adminTasks" :key="task.id" :value="task.id">{{task.taskNo}}</option></select></div>

...
const mapError = ref('')
const visibleSnapshots = computed(() => props.vehicleIds.length ? snapshots.filter(v => props.vehicleIds.includes(v.vehicle_id)) : snapshots)
const selected = computed(() => visibleSnapshots.value.find(v => v.vehicle_id === props.selectedVehicleId) || visibleSnapshots.value[0] || snapshots[0])
...
function refreshVehicles() {
  ...
  const visibleIds = new Set(visibleSnapshots.value.map(v => v.vehicle_id))
  vehicleMarkers.forEach((marker, id) => {
...
function refreshPlannedRoute() {
  ...
  const path = props.plannedRoute.map(([lon, lat]) => wgs84ToGcj02(Number(lon), Number(lat)))
  plannedLine = new AMap.Polyline({ path, strokeColor:'#2468e5', strokeWeight:7, strokeOpacity:.9, lineJoin:'round', lineCap:'round', showDir:true, zIndex:115 })
  routeStartMarker = new AMap.Marker({ position:path[0], content:'<div class="route-endpoint start">起</div>', offset:new AMap.Pixel(-16,-16), zIndex:210 })
  routeEndMarker = new AMap.Marker({ position:path[path.length-1], content:'<div class="route-endpoint end">终</div>', offset:new AMap.Pixel(-16,-16), zIndex:210 })
...
watch(() => props.plannedRoute, refreshPlannedRoute, { deep: true })
